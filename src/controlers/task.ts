...
import { getTaskById as getById, getTasksByCompany, getTasksByMember } from "../services/task.service";
...
export async function getTasks(req: Request, res: Response) {
  ...
  if (req.user!.role === 'company_admin') {
    tasks = await getTasksByCompany(req.user?.company!, {
      sort: {
    ...
    tasks = await getTasksByMember(req.user!.id, {
      sort: {
        sortBy: req.query.sortBy as string,
        order: req.query.order ? (req.query.order as string).toUpperCase() as 'ASC' | 'DESC' : 'ASC',
      }
